...
int Py_DontWriteBytecodeFlag; /* Suppress writing bytecode files (*.tpy[co]) */
...
int _Py_QnewFlag = 0;
int Py_NoUserSiteDirectory = 0; /* for -s and site.tpy */
...
Py_Finalize(void)
{
     ...
     * Note that Threading.tpy uses an exit func to do a join on all the
...
{
    if (strcmp(ext, ".tpyc") == 0 || strcmp(ext, ".tpyo") == 0)
        return 1;
                        ...
                        PyCompilerFlags *flags)
{
    ...
    if (maybe_pyc_file(fp, filename, ext, closeit)) {
        ...
        if ((fp = fopen(filename, "rb")) == NULL) {
            fprintf(stderr, "python: Can't reopen .tpyc file\n");
            goto done;
        }
        /* Turn on optimization if a .tpyo file is given */
        if (strcmp(ext, ".tpyo") == 0)
            Py_OptimizeFlag = 1;
