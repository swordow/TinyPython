...
/* Magic word to reject .tpyc files generated by other Python versions.
   It should change for each incompatible change to the bytecode.

   The value of CR and LF is incorporated so if you ever read or write
   a .tpyc file in text mode the magic number will be wrong; also, the
...
_PyImport_Init(void)
{
    ...
    if (Py_OptimizeFlag) {
        /* Replace ".tpyc" with ".pyo" in _PyImport_Filetab */
        for (; filetab->suffix != NULL; filetab++) {
#ifndef RISCOS
            if (strcmp(filetab->suffix, ".tpyc") == 0)
                filetab->suffix = ".tpyo";
#else
            if (strcmp(filetab->suffix, "/tpyc") == 0)
                filetab->suffix = "/tpyo";
...
#ifdef MS_WINDOWS
    /* Treat .tpyw as if it were .tpy.  The case of ".tpyw" must match
       that used in _PyImport_StandardFiletab. */
    if (len >= 4 && strcmp(&pathname[len-4], ".tpyw") == 0)
...
#endif
    if (sizeof mtime > 4) {
        /* Python's .tpyc timestamp handling presumes that the timestamp fits
            ...
            size_t buflen, FILE **p_fp, PyObject **p_loader)
{
    ...
    for (i = 0; i < npath; i++) {
        ...
        if (isdir(buf) &&         /* it's an existing directory */
            ...
            if (find_init_module(buf)) { /* and has __init__.tpy */
                Py_XDECREF(copy);
                PyMem_FREE(name);
                return &fd_package;
            }
 ...
 *    /a/b/c/d/e/f/g/h/i/j/k/some_long_module_name.tpy\0
 *    ^                      ^                   ^    ^
 *    |--------------------- buf ---------------------|
 *    |------------------- len ------------------|
 *                           |------ name -------|
...
#ifdef HAVE_STAT
/* Helper to look for __init__.tpy or __init__.tpy[co] in potential package */
static int
...
/* Return the package that an import is being performed in.  If globals comes
   ...
   sys.modules entry for foo.bar.  If globals is from a package's __init__.tpy,
...
"get_magic() -> string\n\
Return the magic number for .tpyc or .tpyo files.");
